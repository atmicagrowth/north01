 ...
 * sentences the code did not back, and the privacy notice again on 2026-09-15 for the demonstration
 * notice's storage (DEV-86). It lives in `site-settings` (Site Settings → Policies) so the owner
 ...
 * | A note that the demonstration notice was seen | `lib/demo-notice.ts` `DEMO_NOTICE_KEY` — `localStorage` (DEV-86) |
...
export const PRIVACY_PARAGRAPHS: readonly string[] = [
  'NORTH / 01 is an online-only clothing shop. This notice explains what personal information we collect when you browse or buy, why we hold it, who else receives it, and how long we keep it. Last updated 15 September 2026.',
  ...
  'Cookies and device storage. We set two cookies of our own: one keeps you signed in, for up to seven days, and one identifies your bag, for thirty days. Your browser’s local storage for this site keeps the products you save without signing in, the products you have recently viewed, and your recent searches. Those lists stay on your device: your browser sends us product codes from them only so that we can show you those products, and we do not keep them, except that products you saved move to your account when you sign in. Local storage also keeps a note that you have pressed Continue on the notice saying this website is a demonstration, so that it is not shown again. Session storage holds a note that stops a purchase being counted twice, and is cleared when you close the tab. Where measurement is switched on, Google Analytics sets its own cookies, which can last up to two years, and PostHog keeps its identifier in a cookie that lasts a year from your last visit, and in local and session storage. You can clear all of this by clearing this site’s data in your browser; the sign-in and bag cookies are what make the shop work.',
