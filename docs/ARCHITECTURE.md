...
| G-08 | **About**, **Order Tracking**, and the support surface (**FAQ / Contact / Shipping / Returns**) are navigation destinations in the structure doc with no dedicated implementation phase. | ~~Phase 23~~ **Closed 2026-09-11 (Phase 37).** FAQ, Shipping and Returns render at `/help/faq`, `/help/shipping` and `/help/returns` (Phase 28, notes §1.33). **Contact** is a published support address, `admin@micagrowth.com`, supplied by the owner on 2026-09-11: it is `siteSettings.contactEmail` (the reply-to on order emails) and is named by the returns policy, the FAQ, the terms and the privacy notice. There is no contact form, by decision, so `contactConfirmation` stays unwired (DEV-66). About is withdrawn (**DEV-07**, amended in Phase 30); Order Tracking is withdrawn (**DEV-77**). |
...
| G-19 | **Privacy policy and terms.** The footer's legal links (feature matrix §1, structure §20) need legal text somebody accountable has to write. | **Closed 2026-09-11 (Phase 37)** — the owner supplied the contact address and the retention decision and asked for the text, which was drafted from the code (`scripts/seed/legal.ts`, revised 2026-09-13) as an unreviewed starting draft, not legal advice. It lives in `site-settings` → Policies (editable in the admin) and renders at `/legal/privacy` and `/legal/terms`. **Each document is published only while it has text** (`hasPublishedText`): an empty one answers 404, and the footer's `legalNav` row, the Support nav, the checkout acceptance sentence and the sitemap all drop its link. Production shows neither until the text is pasted into its admin (TODO.md §12). Still the owner's: the registered company name and address (both documents) and the governing law (the terms), and a review by somebody qualified (TODO.md §9). |
| G-20 | **Online returns.** Structure §2 promises an *"online return request/support flow"*, and no phase built one: no return-request action on an order, no record, no email, no guest path. | **Open** — recorded in Phase 36 (audit DOC-01). Since Phase 37 returns are arranged **by email** with the team at `admin@micagrowth.com`, which the returns policy, the FAQ and the terms all name, and no page promises a self-service return. That is the interim channel, not the flow structure §2 specifies: a return request on shipped or delivered orders inside the window, with a record, an email and a guest path, is still not built. |
