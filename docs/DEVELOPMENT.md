...
| `pnpm content:publish` / `pnpm content:publish:write` | Report, then write, the owner's support and legal copy — contact address, shipping and returns policies, the privacy notice and terms, the six FAQ answers and the two size-guide fit notes — into whichever database the shell names. The strings come from `scripts/seed/support.ts` and `scripts/seed/legal.ts`, the same modules the seed writes from, so the live shop cannot disagree with the repository. It creates nothing, skips what already matches, and is meant to be pointed at production from a shell holding its `DATABASE_URL` for that command only. Two files rather than a flag: `payload run` forwards no arguments |
