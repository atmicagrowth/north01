...
| `pnpm verify:home` | The Phase 10 homepage rules — the block resolver's drop/keep behaviour for every edge case in feature matrix §3, the rail queries, price formatting, the rich-text href rule, the LCP and heading choices, and that no generated Postgres identifier has been truncated at 63 bytes. Creates real campaigns and products for the publication cases, then removes them; local database only |
| `pnpm verify:catalog` | The Phase 11 catalogue rules — every §11.1d filter edge case, the nine §11.1b card states, badge precedence, category subtree expansion, the URL contract, the index-record rules, and **that the Postgres and Algolia engines return the same products in the same order** for every query both can answer. Creates a draft, a scheduled and a withdrawn product, then removes them; local database only |
| `pnpm verify:search` | The Phase 12 search rules — pure fixtures first, then **real Payload documents**, then a live section that runs only when Algolia is configured and records an explicit skipped-and-passing check when it is not. Local database only |
| `pnpm verify:product` | The Phase 13 product-detail rules — the variant matrix, the roving-focus selector, the size guide, the media grid sizes, and every unavailable-variant state. Creates its own product and variants, then removes them; local database only |
| `pnpm verify:cart` | The Phase 14 bag rules — quantity clamping against live stock and the per-line ceiling, line merging, totals, the free-shipping progress, and the guest-to-customer merge. Local database only |
| `pnpm verify:promotions` | The Phase 15 promotion rules — all eight validity checks in the order they are reported, eligibility through the collection join, rounding, and the discount never exceeding what it discounts. Local database only |
| `pnpm verify:shipping` | The Phase 16 shipping and tax boundaries — the rate card, the free-shipping threshold read against the **discounted** subtotal, unsupported destinations, and the deferred tax provider answering `unavailable` rather than inventing a number |
| `pnpm verify:checkout` | The Phase 17 checkout decisions — the payment state machine, preflight's failure vocabulary, the order-number alphabet, the first idempotency barrier against the real unique constraint, the delivery counter under concurrency, and **Stripe signature verification offline** using the SDK's own test-header signer, with a tampered payload and a wrong secret both refused |
| `pnpm verify:webhook` | The Phase 17 webhook **effects** against the real database — both idempotency barriers, the third (the order is *claimed*, not checked), §17.1f's inventory race run concurrently, the paid-but-unfulfilled path, and §17.1h's failure list. Creates real orders, lines and variants, then removes them; local database only |
| `pnpm verify:orders` | The Phase 18 order system — §18.1b's fulfilment machine over all 25 ordered pairs, §18.1c's tracking conditions and its authorization (anonymous, customer and staff attempts against the live access layer), §18.1d's snapshots proved by renaming and repricing the product and then by trying to retype the line, DEV-03's derived display status over all 35 combinations, and the `charge.refunded` path resolved by payment intent. Since Phase 37, **section M** (17 checks) proves the retention sweep against real rows: its own order fixtures dated 1997–2000, every sweep run at a fixed `SWEEP_NOW` clock (2000-03-01, cutoff 2000-01-31) so no real or seeded order can ever match, fixtures deleted permanently (`trash: true`) when the run ends, and leftovers from an aborted run (`N1-ORD-M…`, `orders@example.test`, dated before 2001) cleared first. Local database only |
| `pnpm content:publish` / `pnpm content:publish:write` | Report, then write, the owner's support and legal copy — contact address, shipping and returns policies, the privacy notice and terms, all fourteen FAQ answers and the two size-guide fit notes — into whichever database the shell names. The strings come from `scripts/seed/support.ts`, `scripts/seed/editorial.ts` and `scripts/seed/legal.ts`, the same modules the seed writes from, so the live shop cannot disagree with the repository. It creates nothing, skips what already matches, and is meant to be pointed at production from a shell holding its `DATABASE_URL` for that command only. Two files rather than a flag: `payload run` forwards no arguments |
