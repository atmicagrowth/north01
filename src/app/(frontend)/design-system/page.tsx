...
export default function DesignSystemPage() {
  return (
    /*
      The shell is no longer rendered here. Phase 9 mounts the real header and footer in the
      storefront root layout, so this route is inside them like every other page — which is a better
      specimen than a second copy: what a reviewer sees on this sheet is now literally the shipped
      shell, on the shipped data, with the shipped overlays.

      The `<main>` landmark left with them. The layout declares exactly one.
    */
    <PageContainer>
      ...
      >
        ...
        >
            ...
            {[
              ...
              <div key={swatch.name} className="flex flex-col gap-2">
                ...
                <div className="flex flex-col gap-0.5">
                  <p className="font-sans text-micro uppercase text-foreground">{swatch.name}</p>
          ...
          <div className="mt-m flex flex-col gap-2 rounded-sm border border-border p-m">
            <p className="font-sans text-meta uppercase text-foreground">Two rules worth stating</p>
            <p className="max-w-measure font-sans text-body-sm text-foreground-muted">
              ...
              1.53:1 — fine for a divider, which WCAG treats as decorative, and far below the 3:1 a
              control boundary needs. Structural rules stay Graphite; anything a customer has to
              find and operate uses Muted Stone.
            </p>
            <p className="max-w-measure font-sans text-body-sm text-foreground-muted">
              <strong className="text-foreground">There is no third grey for text.</strong> Muted
              Stone is 4.15:1, below AA for normal text, so it answers only to <em>disabled</em> —
              where WCAG 1.4.3 exempts it. Everything below primary is Stone at 7.91:1, and the
              hierarchy is carried by size and tracking instead. A &ldquo;tertiary text&rdquo; alias
              existed briefly; every one of its first uses broke its own restriction, so it was
              deleted rather than re-documented.
            </p>
          </div>
        </Specimen>
        ...
        >
          <div className="flex flex-col gap-l">
            {[
              ...
              <div key={level.token} className="flex flex-col gap-2 border-b border-border pb-m">
                <div className="flex flex-wrap items-baseline gap-m">
                  <code className="font-mono text-micro uppercase text-accent">{level.token}</code>
          ...
          <p className="mt-m max-w-measure font-sans text-body-sm text-foreground-muted">
            ...
            finer hairlines than the smaller ones — the browser applies it automatically. That is
            the difference between a fashion Didone and a general-purpose serif, and it only works
            because the committed file is the one that contains the axis.
          </p>
        </Specimen>
        ...
        >
          ...
          <p className="mt-m max-w-measure font-sans text-body-sm text-foreground-muted">
            ...
            <code className="font-mono text-body-sm text-foreground">prefers-reduced-motion</code>,
        ...
        >
          <Matrix>
            ...
            <Cell label="Radio group" wide>
              <RadioGroup defaultValue="standard" aria-label="Shipping method" className="w-full">
                {[
          ...
          <Matrix>
            ...
            <Cell label="Compare-at price — the G-14 answer" wide>
              <div className="flex flex-col gap-2">
                ...
                <p className="max-w-measure font-sans text-body-sm text-foreground-muted">
                  The live price stays Bone; the struck-through price is Stone at 7.91:1, not the
                  dimmer tertiary tone, because it is still information a customer reads. No red, no
                  fill — the strike and the label carry it.
                </p>
              </div>
            </Cell>
          </Matrix>
      ...
      >
          ...
          <div className="flex flex-col gap-l">
            {(['media-start', 'media-end'] as const).map((layout) => (
              <div key={layout} className="rounded-sm border border-border p-m">
                ...
                >
                  <p className="font-sans text-meta uppercase text-foreground-muted">{layout}</p>
        ...
        <Specimen
          id="shell"
          name="Global shell"
          note="Mounted for real, at the top and bottom of this page. Phase 9 drives it from the
                    Payload navigation and site-settings globals."
        >
          <p className="max-w-measure font-sans text-body-sm text-foreground-muted">
            The header above this sheet is the shipped header: its primary items, mega-menu columns,
            featured panel, footer columns and social links all come from the CMS, and an item whose
            target is unpublished or deleted is dropped rather than rendered as a dead link. Open
            the search overlay and the bag from the header to see the shell&rsquo;s overlay state
            machine — opening either closes the other, and the mega menu, and the mobile drawer.
          </p>
        </Specimen>
        ...
        >
          <p className="max-w-measure font-sans text-body-sm text-foreground-muted">
            ...
            below 640px, so this whole sheet doubles as the mobile-width check that plan §3.1d asks
            for. The drawer specimens above are the ones to open on a phone — they are what the
            mobile navigation and the cart become.
          </p>
        </Specimen>
      </SpecimenGroup>
